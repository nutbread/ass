...
def function_header(method_object, method_info, return_type, params):
	global __section_name;
	assert(__section_name is not None);
	...
	# Params
	src.append(
		'<code class="doc_head doc_params_block{1:s}">'.format(function_id, " doc_params_none" if (len(params) == 0) else "")
	);
...
def member_header(member_object, member_info, extra_info):
	...
	if (len(member_infos) == 1):
		main_id = "{0:s}.{1:s}".format(__section_name, member_infos[0][0]);
	else:
		main_id = "{0:s}.{1:s}".format(__section_name, extra_info["id"]);
